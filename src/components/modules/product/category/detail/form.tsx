...
export default function ProductCategoryForm(props: FormProps) {
  ...
  const [categoryList, setCategoryList] = useState<null | number[]>(null);
  ...
  const entityEdit = useRef<ProductCategoryType | null>(productCategory ?? null);
  ...
  const validationSchema = yup.object({
    ...
    parentId: yup.number().nullable().notRequired(),
  });
  ...
  const {
    ...
    getValues,
    formState: { errors },
  } = useForm({ resolver: yupResolver(validationSchema), defaultValues: { parentId: null } });
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <Box sx={{ mb: theme.spacing(4) }}>
            <Popper
              ...
              anchorEl={anchorEl}
              setCategory={(e: number) => setValue("parentId", e)}
            />
            <TextField
              label={"parentIdd"}
              fullWidth
