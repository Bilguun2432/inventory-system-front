...
export default function TranslationForm(props: FormProps) {
  const [entityLoading, setEntityLoading] = useState<boolean>(true);
  const { id, id2, locale, productCategory, onComplete } = props;
  ...
  const { error, isLoading } = useDetailSwr(id2 ? id2 : id && id > 0 ? id : 0);

  const { data } = useDetailTranslateSwr(id && id > 0 ? id : 0, locale);
